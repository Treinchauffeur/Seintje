...
public class CombinatieSeinenDialog extends Dialog implements View.OnClickListener {

    protected final Context context;
    protected final MainActivity activity;
    protected ViewGroup allViews;
    public static final String TAG = "CombinatieDialog";

    public CombinatieSeinenDialog(@NonNull Context context, MainActivity activity) {
        super(context);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.dialog_combinatie);

        allViews = findViewById(R.id.combinatieDialogCardView);
        ...
        for (View v : allViews.getTouchables()) {
            if (v instanceof Button && v.isClickable()) {
                ...
                if (Constants.DEBUG) {
                    ...
                    if (preConvertedName.toLowerCase().contains("sein"))
                        convertedDrawableResourceString = "rail_piece_seinbordcombinatie_" + context.getResources().getResourceEntryName(v.getId()).split("sein")[1].toLowerCase();
                    else
    ...
    public void onClick(View view) {
        ...
        if (preConvertedName.toLowerCase().contains("sein"))
            convertedDrawableResourceString = "rail_piece_seinbordcombinatie_" + context.getResources().getResourceEntryName(view.getId()).split("sein")[1].toLowerCase();
        else
            convertedDrawableResourceString = "";
